...
$labels = $is_myanmar_language
    ? [
        ...
        'unit_type'              => 'ယူနစ်အသုံးပြုပုံ',
        'normal_unit'            => 'သာမန်ယူနစ်',
        'dual_unit'              => 'Dual Unit (အလေးချိန်နှင့် အရေအတွက် သီးခြား)',
        'dual_unit_help'         => 'Dual Unit ကိုရွေးပါက အခြေခံအလေးချိန်နှင့် လုံး/ခွေ အရေအတွက်ကို သီးခြားမှတ်တမ်းတင်မည်။',
        'secondary_unit'         => 'သီးခြားရေတွက်ယူနစ် (Count Unit)',
        'secondary_unit_help'    => 'ဥပမာ - လုံး၊ ခွေ။ ဤယူနစ်ကို အလေးချိန် conversion အဖြစ် မတွက်ပါ။',
    ...
    : [
        ...
        'unit'                   => 'Product Unit',
        'unit_type'              => 'Unit Tracking Type',
        'normal_unit'            => 'Normal Unit',
        'dual_unit'              => 'Dual Unit (independent weight and count)',
        'dual_unit_help'         => 'Dual Unit tracks the primary weight and piece/coil count independently.',
        'secondary_unit'         => 'Independent Count Unit',
        'secondary_unit_help'    => 'Example: Piece or Coil. This unit is not treated as a weight conversion.',
...
$unit_type_dropdown = [
    '0' => $labels['normal_unit'],
    '1' => $labels['dual_unit'],
];
$selected_unit_type = set_value(
    'is_dual_unit',
    !empty($product->is_dual_unit) ? '1' : '0'
);
$selected_secondary_unit = set_value(
    'secondary_unit_id',
    $product->secondary_unit_id ?? ''
);
...
$product_page_labels = [
    ...
    'dual_secondary_required'     => $is_myanmar_language ? 'Dual Unit အတွက် သီးခြားရေတွက်ယူနစ်ကို ရွေးပါ။' : 'Select an independent count unit for the Dual Unit product.',
    'dual_units_must_differ'      => $is_myanmar_language ? 'အခြေခံယူနစ်နှင့် သီးခြားရေတွက်ယူနစ် မတူရပါ။' : 'The primary unit and independent count unit must be different.',
    'product_name_required'       => $is_myanmar_language ? 'ကုန်ပစ္စည်းအမည်ကို ဖြည့်ပါ။' : 'Enter the product name.',
    'product_code_required'       => $is_myanmar_language ? 'ကုန်ပစ္စည်းကုဒ်ကို ဖြည့်ပါ။' : 'Enter the product code.',
    'category_required'           => $is_myanmar_language ? 'ကုန်ပစ္စည်းအုပ်စုကို ရွေးပါ။' : 'Select a product category.',
    'primary_unit_required'       => $is_myanmar_language ? 'ကုန်ပစ္စည်းအခြေခံယူနစ်ကို ရွေးပါ။' : 'Select the primary product unit.',
    'alert_quantity_invalid'      => $is_myanmar_language ? 'အနည်းဆုံးလက်ကျန်သတိပေးအရေအတွက်ကို ဂဏန်းဖြင့် ဖြည့်ပါ။' : 'Enter a valid stock alert quantity.',
];
?>
...
<style>
    ...
    .dual-unit-panel.is-dual .dual-unit-status {
        background: #ede9fe;
        color: #6d28d9;
    }
    ...
    .erp-input-group .btn:hover,
    .erp-input-group .btn:focus {
        background: #0f766e !important;
        color: #fff !important;
        box-shadow: 0 5px 12px rgba(15, 118, 110, .16);
    }

    .erp-input-group .btn i {
        margin-right: 5px;
    }
    ...
    <div class="erp-page">
        <div class="erp-card erp-form">
            ...
            <div class="erp-body">
                <?php
                /*
                 * Select2 က မူလ select ကို ဖျောက်ထားသဖြင့် browser native
                 * required validation သုံးလျှင် "not focusable" ဖြစ်ပြီး
                 * submit request မထွက်နိုင်ပါ။ ဒီ form ကို အောက်ရှိမြင်သာသော
                 * JavaScript validation နှင့် Controller validation ဖြင့်စစ်မည်။
                 */
                ?>
                <?= form_open_multipart(
                    'products/edit/' . (int) $product->id,
                    'class="erp-product-edit-form" id="product_edit_form" novalidate="novalidate"'
                ); ?>

                <input type="hidden" name="save_action" id="save_action" value="save">

                <!-- Edit မှာ မပြထားသည့် field များ၏ လက်ရှိတန်ဖိုး မပျက်စေရန် preserve လုပ်ထားသည် -->
                <input type="hidden" name="type" value="<?= html_escape($product->type ?? 'standard'); ?>">
                <input type="hidden" name="barcode_symbology" value="<?= html_escape($product->barcode_symbology ?? 'code128'); ?>">
                <input type="hidden" name="cost" value="<?= html_escape($product->cost ?? 0); ?>">
                <input type="hidden" name="price" value="<?= html_escape($product->price ?? 0); ?>">
                <input type="hidden" name="product_tax" value="<?= html_escape($product->tax ?? ($product->product_tax ?? ($product->tax_rate ?? 0))); ?>">
                <input type="hidden" name="tax_method" value="<?= html_escape($product->tax_method ?? 0); ?>">
                <input type="hidden" name="has_unit_conversion" id="has_unit_conversion" value="<?= !empty($product->has_unit_conversion) ? '1' : '0'; ?>">

                <!-- ဒီ minimal Edit page မှာ conversion rows မပြထားသဖြင့် လက်ရှိ conversion များကို မပျက်အောင် ပြန်ပို့ထားသည် -->
                <?php if (!empty($product_conversions)) { ?>
                    <?php foreach ($product_conversions as $conversion) { ?>
                        <span class="preserved-conversion-row" data-unit-id="<?= (int) $conversion->unit_id; ?>">
                            <input type="hidden" name="conversion_unit_id[]" value="<?= (int) $conversion->unit_id; ?>">
                            <input type="hidden" name="operator[]" value="<?= html_escape($conversion->operator); ?>">
                            <input type="hidden" name="operation_value[]" value="<?= html_escape($conversion->operation_value); ?>">
                        </span>
                    <?php } ?>
                <?php } ?>

                <!-- Product Edit မှ stock မပြင်ပါ။ လက်ရှိ stock ကို hidden field ဖြင့်
                     ပြန်ပို့ပါက updateStoreStock() က ထပ်ပေါင်း၍ quantity ၂ ဆဖြစ်နိုင်သည်။ -->
                        ...
                        <div class="col-md-6">
                            <div class="form-group form-group-lg">
                                ...
                                <?= form_dropdown(
                                    'category',
                                    $category_dropdown,
                                    set_value('category', $product->category_id ?? ''),
                        ...
                        <div class="col-md-6">
                            <div class="form-group form-group-lg">
                                ...
                                <?= form_dropdown(
                                    'brand_id',
                                    $brand_dropdown,
                                    set_value('brand_id', $product->brand_id ?? ''),
                ...
                <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group form-group-lg">
                                ...
                                <?= form_input(
                                    'alert_quantity',
                                    set_value('alert_quantity', $product->alert_quantity ?? 0),
                                    'class="form-control tip" id="alert_quantity" required="required" placeholder="0"'
                                ); ?>
                ...
                <div id="dual_unit_panel" class="dual-unit-panel<?= (string) $selected_unit_type === '1' ? ' is-dual' : ''; ?>">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group form-group-lg">
                                <label for="is_dual_unit">
                                    <?= html_escape($labels['unit_type']); ?>
                                    <span class="erp-required">*</span>
                                </label>
                                ...
                                <?= form_dropdown(
                                    ...
                                    $unit_type_dropdown,
                                    $selected_unit_type,
                                    'class="form-control tip" id="is_dual_unit" required="required" style="width:100%;"'
                                ); ?>

                                <span id="dual_unit_status" class="dual-unit-status">
                                    <i class="fa fa-cube"></i>
                                    <span><?= html_escape((string) $selected_unit_type === '1' ? $labels['dual_unit'] : $labels['normal_unit']); ?></span>
                                </span>
                                <span class="erp-help"><?= html_escape($labels['dual_unit_help']); ?></span>
                            </div>
                        </div>

                        <div class="col-md-6" id="secondary_unit_group"<?= (string) $selected_unit_type === '1' ? '' : ' style="display:none;"'; ?>>
                            <div class="form-group form-group-lg">
                                <label for="secondary_unit_id">
                                    <?= html_escape($labels['secondary_unit']); ?>
                                    <span class="erp-required">*</span>
                                ...
                                <?= form_dropdown(
                                    'secondary_unit_id',
                                    $unit_dropdown,
                                    $selected_secondary_unit,
                                    'class="form-control select2 erp-select2 tip" id="secondary_unit_id" style="width:100%;"' .
                                    ((string) $selected_unit_type === '1' ? ' required="required"' : '')
                                ); ?>

                                <span class="erp-help"><?= html_escape($labels['secondary_unit_help']); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group form-group-lg">
                        ...
                        <?= form_textarea(
                            'details',
                            set_value('details', $product->details ?? ''),
                ...
                <div class="erp-actions">
                        ...
                        <button type="submit" name="edit" value="edit" data-action="save"
...
<script type="text/javascript" charset="utf-8">
    /*
     * App/mobile navigation က edit view အဟောင်းကို DOM ထဲမှာ သိမ်းထားနိုင်သည်။
     * အဲဒီအခါ product_edit_form, code နဲ့ quick modal ID တွေ ထပ်သွားပြီး
     * ဒုတိယအကြိမ် Save နှိပ်ရာမှာ handler က form အဟောင်းကို ကိုင်မိသည်။
     * လက်ရှိ render (နောက်ဆုံး element) ကိုသာထားပြီး အဟောင်းများကို ရှင်းမည်။
     */
    (function cleanupPreviousProductEditView() {
        var selectors = [
        ...
        /* Modal အဟောင်းမှ ကျန်ခဲ့သော overlay/scroll lock ကိုလည်း ရှင်းမည်။ */
        if (!document.querySelector('.modal.in, .modal.show')) {
            var backdrops = document.querySelectorAll('.modal-backdrop');

            for (var j = 0; j < backdrops.length; j++) {
                if (backdrops[j] && backdrops[j].parentNode) {
    ...
    var originalProductCode = <?= json_encode(trim((string) ($product->code ?? '')), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
    var checkCodeTimer = null;

    $(document).ready(function () {

        var quickUnitTarget = '#unit';
        ...
        function syncPreservedConversions() {
            var isDual = $('#is_dual_unit').val() === '1';
            var secondaryUnitId = String($('#secondary_unit_id').val() || '');
            var enabledConversions = 0;

            $('.preserved-conversion-row').each(function () {
                var $row = $(this);
                var isIndependentCountConversion =
                    isDual &&
                    secondaryUnitId !== '' &&
                    String($row.data('unit-id')) === secondaryUnitId;

                $row.find('input').prop('disabled', isIndependentCountConversion);
                if (!isIndependentCountConversion) {
                    enabledConversions++;
                }
            });

            $('#has_unit_conversion').val(enabledConversions > 0 ? '1' : '0');
        }

        function syncDualUnitFields() {
            var isDual = $('#is_dual_unit').val() === '1';
            var statusText = $('#is_dual_unit option:selected').text();

            $('#dual_unit_panel').toggleClass('is-dual', isDual);
            $('#secondary_unit_group').toggle(isDual);
            $('#secondary_unit_id').prop('required', isDual);
            $('#dual_unit_status span').text(statusText);

            syncPreservedConversions();
        }

        $('#is_dual_unit, #secondary_unit_id, #unit').on('change', function () {
            syncDualUnitFields();
        });

        $('[data-target="#quickUnitModal"]').on('click', function () {
            var selectId = $(this).closest('.form-group').find('select').attr('id');
            quickUnitTarget = selectId ? '#' + selectId : '#unit';
        });
        ...
        function checkProductCode() {
            ...
            // မူလ product code ကို မပြောင်းထားလျှင် ကိုယ့် code ကို duplicate မစစ်ရပါ။
        ...
        function showProductFormError(message, selector) {
            var $field = $(selector);
            var node = $field.get(0);
            ...
            window.setTimeout(function () {
                if ($field.is('select') && $.fn.select2) {
                    try {
                        $field.select2('open');
                        return;
                    } catch (select2OpenError) {}
                }
        ...
        $('#product_edit_form').on('submit', function (e) {
            ...
            var isDual = $('#is_dual_unit').val() === '1';
            var primaryUnitId = String($('#unit').val() || '');
            var secondaryUnitId = String($('#secondary_unit_id').val() || '');
            var alertQuantity = $.trim($('#alert_quantity').val());

            $code.val(trimmedCode);

            if (productName === '') {
                e.preventDefault();
                showProductFormError(productPageLabels.product_name_required, '#name');
                return false;
            }

            if (trimmedCode === '') {
                e.preventDefault();
                showProductFormError(productPageLabels.product_code_required, '#code');
                return false;
            }

            if (categoryId === '') {
                e.preventDefault();
                showProductFormError(productPageLabels.category_required, '#category');
                return false;
            }

            if (primaryUnitId === '') {
                e.preventDefault();
                showProductFormError(productPageLabels.primary_unit_required, '#unit');
                return false;
            }

            if (alertQuantity !== '' && !$.isNumeric(alertQuantity)) {
                e.preventDefault();
                showProductFormError(productPageLabels.alert_quantity_invalid, '#alert_quantity');
                return false;
            }

            if (isDual && secondaryUnitId === '') {
                e.preventDefault();
                showProductFormError(productPageLabels.dual_secondary_required, '#secondary_unit_id');
                return false;
            }

            if (isDual && primaryUnitId === secondaryUnitId) {
                e.preventDefault();
                showProductFormError(productPageLabels.dual_units_must_differ, '#secondary_unit_id');
                return false;
            }

            if (!isDual) {
                $('#secondary_unit_id').val('');
            }
            ...
            if (codeIsDuplicate) {
                e.preventDefault();
                showProductFormError(productPageLabels.product_code_exists_change, '#code');
                return false;
            }
        ...
        $('#quick_unit_form').on('submit', function (e) {
            ...
            postAjax(site_url + 'products/quick_add_unit', {
                ...
                if (res.status === 'success') {
                    $('#unit, #secondary_unit_id').each(function () {
                        $(this).append(
                            $('<option>', {
                                value: res.id,
                                text: res.name
                    ...
                    setSelectValue(quickUnitTarget, res.id);
                    syncDualUnitFields();
