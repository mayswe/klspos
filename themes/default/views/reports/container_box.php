...
if ($this->input->post('container_box')){
    $v .= "&container_box=".$this->input->post('container_box');
}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table = $('#ExRData').DataTable({
            'ajax' : { url: '<?=site_url('reports/get_container_box/'.$v);?>', type: 'POST', "data": function ( d ) {
...
<section class="content">
                ...
                <div class="box-body">
                    <div id="form" class="panel panel-warning">
                        <div class="panel-body">
                            <?= form_open("reports/container_box");?>
                            <div class="row">
                                <div class="col-xs-4">
                                    <div class="form-group form-group-lg">
                                        <label class="control-label" for="container_box"><?= lang("container_box"); ?></label>
                                        <?php
                                        $pr[0] = lang("select")." ".lang("categories");
                                        foreach($container_boxes as $container_box){
                                            $pr[$container_box->id] = $container_box->box_name;
                                        }
                                        echo form_dropdown('container_box', $pr, set_value('container_box'), 'class="form-control select2" style="width:100%" id="container_box"');
                                        ?>
                                    </div>
                                </div>
    ...
    <tr>
        <th></th> <!-- id -->
        <th><input type="text" class="text_filter" placeholder="<?= lang("container_box"); ?>" /></th>
