...
<?php if (!empty($all_invoices)) : ?>
    <?php $first_customer = $all_invoices[0]['customer']; ?>
    <div style="margin-bottom: 15px; font-size: 16px; font-weight: bold;">
        <?= lang('customer'); ?>: <?= $first_customer->name; ?>
    </div>
<?php endif; ?>
...
<div style="text-align:right; margin-bottom: 10px; font-size: 14px; font-weight: bold;">
    ...
    <tbody>
        <?php foreach ($all_invoices as $invoice): ?>
            ...
            <tr>
                <td><?= $inv->id; ?></td>
                <td><?= $this->tec->hrsd($inv->date); ?></td>
                <td><?= $this->tec->formatMoney($inv->grand_total); ?></td>
                <td><?= $this->tec->formatMoney($inv->paid); ?></td>
                <td><?= $this->tec->formatMoney($inv->grand_total - $inv->paid); ?></td>
            </tr>
        <?php endforeach; ?>
    </tbody>
</table>
