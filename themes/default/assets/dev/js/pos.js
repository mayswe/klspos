  ...
  if (get('spositems')) {
    ...
    $.each(sortable, function () {
      ...
      if (product_units[product_id]) {
        $.each(product_units[product_id], function(id, code) {
          var selected = (item.row.unit_id == id) ? 'selected' : '';
          unitOptions += '<option value="' + id + '" ' + selected + '>' + code + '</option>';
        });
      }
      ...
      tr_html =
        ...
        ')</span></button></td>';
      tr_html +=
        '<td><select class="form-control pos-unit" data-product-id="' + product_id + '" data-item-id="' + item_id + '">' +
        unitOptions +
        '</select></td>';
      ...
      tr_html +=
        ...
        '</span></td>';
      tr_html += '<td class="text-center"><i class="fa-solid fa-trash tip pointer posdel" id="' + row_no + '" title="Remove"></i></td>';
      newTr.html(tr_html);
...
$(document).on('change', '.pos-unit', function() {
  var $row = $(this).closest('tr');
  var unit_id = $(this).val();
  var product_id = $(this).data('product-id');

  var conversion = product_conversions[product_id][unit_id];
  var base_price = parseFloat($row.find('.realuprice').val());
  var new_price = formatMoney(base_price * conversion);
  $row.find('.sprice').text(new_price);

  var qty = parseFloat($row.find('.rquantity').val());
  $row.find('.ssubtotal').text(formatMoney(qty * new_price));
...
function calculateTotals() {
  ...
  $('#posTable tbody tr').each(function() {
    total += parseFloat($(this).find('.ssubtotal').text().replace(',', '')) || 0;
  });
  $('#total-payable').text(formatMoney(total));
  $('#total').text(formatMoney(total));
}
