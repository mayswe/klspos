...
function dualUnitBaseFactor(productId, unitId, baseUnitId) {
  if (!unitId || String(unitId) === String(baseUnitId)) {
    return 1;
  }

  var conversion = product_unit_conversions?.[productId]?.[unitId];
  if (!conversion) {
    return null;
  }

  var value = parseFloat(conversion.value);
  if (!isFinite(value) || value <= 0) {
    return null;
  }

  return conversion.operator === "/" ? 1 / value : value;
}

function dualUnitBillingQty(row, productId) {
  var primaryQty = Math.max(0, parseFloat(row.primary_qty) || 0);
  var secondaryQty = Math.max(0, parseFloat(row.secondary_qty) || 0);
  var selectedUnitId = row.unit_id || row.base_unit_id;

  if (String(selectedUnitId) === String(row.secondary_unit_id)) {
    return secondaryQty;
  }

  var factor = dualUnitBaseFactor(
    productId,
    selectedUnitId,
    row.base_unit_id
  );

  return factor ? primaryQty / factor : 0;
}
...
function add_invoice_item(item) {
  ...
  // ✅ Store unit prices globally per product
  if (item.unit_prices) {
    var pid = item.item_id.toString();
    product_unit_prices[pid] = {};

    Object.keys(item.unit_prices).forEach(function (unitId) {
      product_unit_prices[pid][unitId.toString()] = item.unit_prices[unitId];
    });
  }

  if (item.unit_conversions) {
    var pid = item.item_id.toString();
    product_unit_conversions[pid] = item.unit_conversions;
  }

  console.log("product_unit_prices now:", product_unit_prices);
  ...
  if (spositems[item_id]) {
    ...
    if (item.row.is_dual_unit == 1) {
      spositems[item_id].row.primary_qty =
        (parseFloat(spositems[item_id].row.primary_qty) || 0) +
        (parseFloat(item.row.primary_qty) || 0);
      spositems[item_id].row.secondary_qty =
        (parseFloat(spositems[item_id].row.secondary_qty) || 0) +
        (parseFloat(item.row.secondary_qty) || 0);
      spositems[item_id].row.order_qty = dualUnitBillingQty(
        spositems[item_id].row,
        item.item_id
      );
    } else {
      spositems[item_id].row.qty =
        parseFloat(spositems[item_id].row.qty) + parseFloat(item.row.qty || 1);
    }
  } else {
  ...
  store("product_unit_prices", JSON.stringify(product_unit_prices));
  store("product_unit_conversions", JSON.stringify(product_unit_conversions));
...
function loadItems() {
  if (get("product_unit_prices")) {
    product_unit_prices = JSON.parse(get("product_unit_prices"));
  }
  if (get("product_unit_conversions")) {
    product_unit_conversions = JSON.parse(get("product_unit_conversions"));
  }
  ...
  if (get("spositems")) {
    ...
    if (Settings.remote_printing != 1) {
      var pos_customer =
        "C: " + $("#select2-spos_customer-container").text() + "\n";
      var hr = "R: " + $("#hold_ref").val() + "\n";
      var user = "U: " + username + "\n";
      var pos_curr_time =
        "T: " +
        date(Settings.dateformat + " " + Settings.timeformat, time) +
        "\n";
      ...
      var pos_head =
        '<span style="text-align:center;"><h3>' + Settings.site_name + "</h3>";
      // var pos_customer = ''; // remove this line and uncomment below to display customer
      var pos_customer =
        "<h5>C: " + $("#select2-spos_customer-container").text() + "</h5>";
    ...
    $.each(sortable, function () {
      ...
      var product_id = item.row.id,
        ...
        item_qty = 0,
        item_aqty = parseFloat(
          item.row.available_qty != null ? item.row.available_qty : item.row.qty
        ),
      ...
      if (item.unit_prices) {
        product_unit_prices[product_id] = item.unit_prices;
      }
      if (item.unit_conversions) {
        product_unit_conversions[product_id] = item.unit_conversions;
      }

      var selected_unit_id = item.row.unit_id || item.row.base_unit_id;
      var primary_unit_id = item.row.base_unit_id;
      var secondary_unit_id = item.row.secondary_unit_id;
      var is_dual_unit = item.row.is_dual_unit == 1;
      var primary_qty = Math.max(0, parseFloat(item.row.primary_qty) || 0);
      var secondary_qty = Math.max(0, parseFloat(item.row.secondary_qty) || 0);

      var unit_price = item.row.real_unit_price && item.row.real_unit_price > 0
        ? parseFloat(item.row.real_unit_price)
        : parseFloat(item.unit_prices?.[item.row.base_unit_id] || 0);

      //var pviss_readonly = item.row.unit_id == 3 ? '' : 'readonly';

      var unit_multiplier = 1;
      let conversion =
        product_unit_conversions?.[product_id]?.[item.row.unit_id];

      if (
        conversion &&
        conversion.operator === "*" &&
        !isNaN(conversion.value) &&
        parseFloat(conversion.value) !== 1
      ) {
        unit_multiplier = parseFloat(conversion.value);
      }

      if (is_dual_unit) {
        item_qty = dualUnitBillingQty(item.row, product_id);
        item.row.order_qty = item_qty;
      } else {
        var ordered_qty = parseFloat(item.row.order_qty);
        if (!isFinite(ordered_qty) || ordered_qty <= 0) {
          ordered_qty =
            selected_unit_id == secondary_unit_id
              ? parseFloat(item.row.qty_secondary)
              : parseFloat(item.row.qty);
        }
        item_qty = isFinite(ordered_qty) ? ordered_qty : 0;
      }

      // Discounts and taxes are calculated from the quantity charged in the
      // currently selected selling unit. Stock quantities are checked below.
      var base_qty = item_qty;
      ...
      if (ds.indexOf("%") !== -1) {
        ...
        if (!isNaN(pds[0])) {
          item_discount = formatDecimal(
            parseFloat((net_price * parseFloat(pds[0])) / 100),
            4
          );
        }
      }
      product_discount += formatDecimal(item_discount * base_qty, 4);
      net_price = formatDecimal(net_price - item_discount, 4);
      //mhsvar base_qty = parseFloat(item_qty) * unit_multiplier;
      ...
      product_tax += formatDecimal(pr_tax_val * base_qty, 4);

      var unitOptions = "";

      // 🔹 Add base unit first
      var baseUnitId =
        base_units[product_id]?.unit_id || item.row.base_unit_id || "";
      var baseUnitName =
        base_units[product_id]?.unit_name ||
        product_units?.[product_id]?.[baseUnitId] ||
        "Unit";
      var selected = item.row.unit_id == baseUnitId ? "selected" : "";
      unitOptions += '<option value="' + baseUnitId + '" ' + selected + ">" + baseUnitName + "</option>";

      // 🔹 Add other units if available (skip if same as base unit)
      if (product_units[product_id] && Object.keys(product_units[product_id]).length > 0) {
          $.each(product_units[product_id], function (id, name) {
              if (id != baseUnitId) {  // avoid duplicate
      ...
      var secondaryUnitName =
        product_units?.[product_id]?.[secondary_unit_id] || "Count";
      var selectedUnitName =
        selected_unit_id == baseUnitId
          ? baseUnitName
          : product_units?.[product_id]?.[selected_unit_id] || "Unit";
      ...
      tr_html =
        '<td style="width:285px;" data-label="Product"><input name="product_id[]" type="hidden" class="rid" value="' +
        product_id +
        '"><input name="sale_item_id[]" type="hidden" value="' +
        ...
        ")</span></button></td>";
        tr_html +=
    '<td style="width:134px; background:yellow;">' +
    (item.row.whbaseqty != null ? parseFloat(item.row.whbaseqty).toFixed(2) : '0.00') +
    ' / ' +
    (item.row.whsecondqty != null ? parseFloat(item.row.whsecondqty).toFixed(2) : '0.00') +
    '</td>' +
    '<td style="width:134px;" data-label="Actual Weight">' +
    (is_dual_unit
      ? '<input type="number" class="form-control text-center dual-primary-qty" ' +
        'name="primary_qty[]" id="primary_qty_' + row_no + '" ' +
        'step="any" min="0" value="' + primary_qty + '">' +
        '<small class="text-muted">' + baseUnitName + ' (base)</small>'
      : '<span class="text-muted">—</span><input type="hidden" name="primary_qty[]" value="">') +
    '</td>' +
    '<td style="width:234px;">' +
    '<select class="form-control pos-unit" ' +
    'data-product-id="' + product_id + '" ' +
    'data-item-id="' + item_id + '" ' +
    'id="unit_select_' + row_no + '">' +
    unitOptions +
    "</select>";
      tr_html +=
        '<input type="hidden" name="unit_id[]" id="unit_id_' +
        row_no +
        '" value="' +
        selected_unit_id +
        '"></td>';
      tr_html +=
  '<td style="width:258px;" class="text-center" data-label="Delivery">' +
    (item.row.cost != null ? parseFloat(item.row.cost).toFixed(2) : '0.00') +
    ' / ' +
    (item.row.delivery != null ? parseFloat(item.row.delivery).toFixed(2) : '0.00') +
  '</td>';
  ...
  var real_unit_price_val =
  item.row.real_unit_price ||
  product_unit_prices[product_id]?.[item.row.unit_id] ||
  net_price;



tr_html +=
  '<td data-label="Unit" class="text-right" style="width:130px">' +
  '<input type="number" step="any" min="0" id="net_price_' + row_no + '" class="form-control kb-pad text-center realuprice" name="real_unit_price[]" value="' +
  real_unit_price_val +
  '" data-item-id="' +
  item.item_id +
  '">' +
  '<input class="rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + ds + '"></td>';
      ...
      tr_html +=
        "<td>" +
        '<input name="item_was_ordered[]" type="hidden" class="riwo" value="' +
        item_was_ordered +
        '">';

      if (is_dual_unit) {
        tr_html +=
          '<input class="form-control kb-pad text-center dual-secondary-qty" name="secondary_qty[]" type="number" ' +
          'value="' + secondary_qty + '" data-item="' + item_id + '" ' +
          'id="secondary_qty_' + row_no + '" step="any" min="0" onClick="this.select();">' +
          '<small class="text-muted">' + secondaryUnitName + '</small>' +
          '<input class="rquantity" name="quantity[]" type="hidden" value="' + item_qty + '" ' +
          'id="quantity_' + row_no + '">' +
          '<div class="dual-billing-qty text-info"><small>Charged: ' +
          formatDecimal(item_qty, 4) + ' ' + selectedUnitName + '</small></div>';
      } else {
        tr_html +=
          '<input type="hidden" name="secondary_qty[]" value="">' +
          '<input class="form-control kb-pad text-center rquantity" name="quantity[]" type="number" ' +
          'value="' + item_qty + '" data-id="' + row_no + '" data-item="' + item_id + '" ' +
          'id="quantity_' + row_no + '" step="0.01" min="0" onClick="this.select();">';
      }

      tr_html += "</td>";
      
      tr_html +='<td class="text-right"><input type="text" class="form-control text-right ssubtotal_input" ' + 'id="subtotal_' + row_no + '" value="' + formatMoney((parseFloat(net_price) + parseFloat(pr_tax_val)) * item_qty) + '"></td>';
      
      tr_html +=
        '<td class="text-center"><i class="fa-solid fa-trash tip pointer posdel" id="' +
        row_no +
        '" title="Remove"></i></td>';
      newTr.html(tr_html);
      newTr.prependTo("#posTable");
      total += formatDecimal(
        (parseFloat(net_price) + parseFloat(pr_tax_val)) * item_qty,
        4
      );
      ...
      if (item_type == "standard" && is_dual_unit) {
        insufficientStock =
          primary_qty > (parseFloat(
            item.row.available_qty != null ? item.row.available_qty : item.row.qty
          ) || 0) ||
          secondary_qty > (parseFloat(item.row.qty_secondary) || 0);
      } else if (item_type == "standard") {
        var selectedFactor = dualUnitBaseFactor(
          product_id,
          selected_unit_id,
          primary_unit_id
        );
        var requestedBaseQty = selectedFactor === null
          ? Number.POSITIVE_INFINITY
          : item_qty * selectedFactor;
        insufficientStock = requestedBaseQty > item_aqty;
      }

      if (item_type == "standard" && insufficientStock) {
        ...
        b_items +=
          printLine(
            item_qty +
              ...
              ": " +
              formatMoney(
                (parseFloat(net_price) + parseFloat(pr_tax_val)) *
                  parseFloat(item_qty)
              )
          ) + "\n";
        o_items +=
    ...
    // Debug: print values used for the availability check
    console.log('STOCK-CHECK',
      {
        product_id: item.item_id,
        product_row_id: item.row.id,
        selected_unit_id: item.row.unit_id,
        item_qty: item_qty,
        item_aqty_raw: item.row.quantity, // server-sent quantity
        item_aqty_parsed: item_aqty,
        conversion: product_unit_conversions?.[String(item.item_id)]?.[String(item.row.unit_id)],
    ...
    $("#ds_con").text(
      "(" + formatMoney(product_discount) + ") " + formatMoney(order_discount)
    );
    ...
    if (Settings.remote_printing != 1) {
      ...
      if (order_tax != 0) {
        b_totals +=
          printLine(lang.order_tax + ": " + formatMoney(order_tax)) + "\n";
      }
      b_totals +=
        printLine(lang.grand_total + ": " + formatMoney(g_total)) + "\n";
...
$(document).on("change", ".ssubtotal_input", function () {
    let row_id = $(this).attr("id").split("_")[1];
    let new_subtotal = parseFloat($(this).val()) || 0;

    let unit_price = parseFloat($("#net_price_" + row_id).val()) || 0;

    if (unit_price > 0) {
        let new_qty = new_subtotal / unit_price;

        $("#quantity_" + row_id).val(formatDecimal(new_qty, 2)).change();
    }
});



$(document).on("change", ".realuprice", function () {
    ...
    // ✅ Update real_unit_price in spositems
    spositems[item_id].row.real_unit_price = new_price;

    // Save back to localStorage
    localStorage.setItem("spositems", JSON.stringify(spositems));

    // ✅ Reload to update subtotal + totals
    loadItems();
});

$(document).on("change", ".dual-primary-qty, .dual-secondary-qty", function () {
    var row = $(this).closest("tr");
    var item_id = row.attr("data-item-id");
    ...
    spositems[item_id].row.primary_qty =
        parseFloat(row.find(".dual-primary-qty").val()) || 0;
    spositems[item_id].row.secondary_qty =
        parseFloat(row.find(".dual-secondary-qty").val()) || 0;
    spositems[item_id].row.order_qty = dualUnitBillingQty(
        spositems[item_id].row,
        spositems[item_id].row.id
    );
...
$(document).on("change", ".rquantity", function () {
  ...
  // Keep stock availability immutable; only the ordered quantity changes.
  spositems[item_id].row.order_qty = qty;
...
$(document).on("change", ".pos-unit", function () {
    ...
    var row_no = row.attr("id");
    $("#unit_id_" + row_no).val(unit_id);

    if (spositems[item_id].row.is_dual_unit == 1) {
        spositems[item_id].row.primary_qty =
            parseFloat(row.find(".dual-primary-qty").val()) || 0;
        spositems[item_id].row.secondary_qty =
            parseFloat(row.find(".dual-secondary-qty").val()) || 0;
    ...
    if (
        product_unit_prices[product_id] &&
        product_unit_prices[product_id][unit_id] !== undefined
    ) {

        var unit_price = parseFloat(product_unit_prices[product_id][unit_id]);
        var multiplier = 1;

        if (
            product_unit_conversions[product_id] &&
            product_unit_conversions[product_id][unit_id]
        ) {

            var conv = product_unit_conversions[product_id][unit_id];

            multiplier =
                conv.operator === "*"
                    ? parseFloat(conv.value)
                    : 1 / parseFloat(conv.value);
        }

        // Update spositems and save
        spositems[item_id].row.unit_id = unit_id;
        spositems[item_id].row.real_unit_price = unit_price;
        spositems[item_id].row.unit_multiplier = multiplier;
        ...
        bootbox.alert("Unit price not found for this product and unit.");
    }
});
...
function calculateTotals() {
  ...
  $("#posTable tbody tr").each(function () {
    total +=
      parseFloat($(this).find(".ssubtotal").text().replace(",", "")) || 0;
  });
...
$(document).ready(function () {
  ...
  $(document).on(
    "click",
    ".suspended_sales .dropdown-menu .header",
    function (e) {
      e.stopPropagation();
    }
  );
    ...
    if (pr_tax !== null && pr_tax != 0) {
      if (parseFloat(item.row.tax_method) == 0) {
        pr_tax_val = formatDecimal(
          (net_price * parseFloat(pr_tax)) / (100 + parseFloat(pr_tax))
        );
        net_price -= pr_tax_val;
        tax = lang.inclusive;
      } else {
  ...
  $(document).on("click", "#editItem", function () {
    ...
    spositems[item_id].row.discount = $("#nDiscount").val()
      ? $("#nDiscount").val()
      : "0";
    ...
    // Delay DOM updates to avoid aria-hidden focus conflict
    setTimeout(function () {
      loadItems();
    }, 300); // 300ms is usually safe for Bootstrap modals
  ...
  $("#reset").click(function (e) {
    ...
    if (protect_delete == 1) {
      var boxd = bootbox.dialog({
        ...
        closeButton: true,
        message:
          '<input id="pos_pin" name="pos_pin" type="password" placeholder="Pin Code" class="form-control kb-pad"> ',
        buttons: {
          danger: {
            label: lang.close,
    ...
    if (count <= 1) {
      ...
      if (Settings.remote_printing == 0) {
        ...
        if (Settings.print_img == 1) {
          ...
          html2canvas(element, { scrollY: 0, scale: 1.3 }).then(function (
            canvas
          ) {
    ...
    if (ds.length != 0) {
      if (apply_to == "order") {
        ...
        if (ds.indexOf("%") !== -1) {
          ...
          $("#ds_con").text(
            "(" +
              formatMoney(product_discount) +
              ") " +
              formatMoney(order_discount)
          );
  ...
  $(document).on("click", ".product", function (e) {
    var code = $(this).val();
    var product_unit_prices = {}; // declare it globally or in a suitable scope
    $.ajax({
      type: "get",
      ...
      success: function (data) {
        if (data !== null) {
          // ✅ Store the unit prices for this product
          product_unit_prices[data.item_id] = data.row.unit_price;
          console.log("AJAX data:", data);
          // ✅ Now add the item to POS
  ...
  $("#payment").click(function () {
    if (count <= 1) {
      ...
      if (Settings.rounding != 0) {
        ...
        $("#twt").text(
          formatMoney(round_total) + " (" + formatMoney(rounding) + ")"
        );
  ...
  $("#preorder-sale").click(function () {
    if (count <= 1) {
        ...
        // Add preorder flag into the form
        $("#pos-sale-form").append('<input type="hidden" name="is_preorder" value="1">');

        // Calculate totals like payment button
        gtotal = formatDecimal(total - order_discount + order_tax);
        if (Settings.rounding != 0) {
            round_total = roundNumber(gtotal, parseInt(Settings.rounding));
            var rounding = formatDecimal(round_total - gtotal);
        ...
        // Open payment modal (same as normal sale)
        $("#payModal").modal({ backdrop: "static" });
    }
});
  ...
  $("#payModal").on("change", "#paid_by", function () {
    ...
    if (Settings.rounding != 0) {
      var rounded_total = formatDecimal(
        roundNumber(gtotal, parseInt(Settings.rounding))
      );
    } else {
  ...
  $("#submit-sale").click(function () {
    ...
    // Trigger the actual submit button once
    $("#submit").click();
});
      ...
      success: function (res) {
        if (res.status == "success") {
          $("#spos_customer").append(
            $("<option></option>").attr("value", res.id).text(res.val)
          );
