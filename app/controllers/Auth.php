...
class Auth extends MY_Controller
{
    ...
    public function __construct()
    {
        ...
        // Google Client Setup
        $this->config->load('google_local');
        $this->google_client = new Google_Client();
        $this->google_client->setClientId('291965783688-lf6dqfbfu70jkjvosrei9i1dcjnnhgpt.apps.googleusercontent.com');
    ...
    public function create_user()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $additional_data = [
                ...
                'store_id'   => $this->input->post('group') == 1 ? 1 : $this->input->post('store_id'),
    ...
    public function mobile_login()
    {
        ...
        $redirect = $redirect ? ltrim($redirect, '/') : 'products/add';
        ...
        if (!$token) {
            $this->session->set_flashdata('error', 'Mobile login token missing. Please sign in again.');
            redirect('login');
        }

        $token_record = $this->db
            ->where('token', $token)
        ...
        if (!$token_record) {
            $this->session->set_flashdata('error', 'Mobile login expired. Please sign in again.');
            redirect('login');
        }

        $user = $this->db
            ->select('id, username, email, active, first_name, last_name, created_on, last_login, last_ip_address, avatar, gender, group_id, store_id')
        ...
        if (!$user || !$user->active) {
            $this->session->set_flashdata('error', 'Mobile login user not found or inactive.');
            redirect('login');
        }
    ...
    public function google_callback()
    {
        if ($this->input->get('code')) {
            ...
            if (!isset($token['error'])) {
                ...
                // Match with existing KLSPOS user
                $user = $this->site->getUserByEmail($email); // already working in your Google login
                
                if ($user && (int) $user->active === 1 && $user_info->verified_email) {
                    if ($this->Settings->mmode && !$this->ion_auth->in_group('admin', $user->id)) {
                        $this->session->set_flashdata('error', lang('site_is_offline_plz_try_later'));
                        redirect('login');
                    }
                    // Login successful
                    ...
                    $this->session->set_flashdata('error', 'This Google account is not registered or active in KLSPOS. Please contact your administrator.');
                    redirect('login');
                }
            } else {
                show_error('Google login error.');
            }
        }
    }
